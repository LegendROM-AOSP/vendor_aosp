...
function setup_vendor() {
    ...
    export LEGENDROM_ROOT="$3"
    if [ ! -d "$LEGENDROM_ROOT" ]; then
        echo "\$LEGENDROM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$LEGENDROM_ROOT/$OUTDIR" ]; then
        mkdir -p "$LEGENDROM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$LEGENDROM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$LEGENDROM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$LEGENDROM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$LEGENDROM_ROOT"/vendor/aosp/build/tools/smali/baksmali.jar
        export SMALIJAR="$LEGENDROM_ROOT"/vendor/aosp/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$LEGENDROM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$LEGENDROM_TARGET" >/dev/null; then
    ...
    for ARCH in $ARCHES; do
        ...
        elif [[ "$LEGENDROM_TARGET" =~ .jar$ ]]; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$LEGENDROM_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$LEGENDROM_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$LEGENDROM_ROOT"/vendor/aosp/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$LEGENDROM_ROOT"/"$OUTDIR"/radio
